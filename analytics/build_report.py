...
def main():
  ...
  ga_query = ga.run_query(ga_service, config.start_date, config.end_date, 'ga:totalEvents', 'ga:eventAction,ga:eventLabel', '')
  orcid_data = orcid.get_orcid_data(repo_dois)
  filedata = ga.create_csv(ga_service, config.report_date_Y_m_d, config.start_date, config.end_date)
